...
def main():
    ...
    if transport == "sse":
        print(f"Starting Cronometer MCP server with SSE transport on {host}:{port}")
        mcp.run(transport="sse", host=host, port=port)
    elif transport == "http" or transport == "streamable-http":
        print(f"Starting Cronometer MCP server with Streamable HTTP transport on {host}:{port}")
